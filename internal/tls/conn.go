...
type Conn struct {
	...
	// readBuf is a reusable buffer for readFromUntil to avoid per-read allocations.
	// Grow once, reuse across all readFromUntil calls for this connection.
	readBuf []byte
...
// setTrafficSecret sets the encryption state for TLS 1.3.
...
func (c *Conn) readRecordOrCCS(expectChangeCipherSpec bool) error {
	...
	case recordTypeChangeCipherSpec:
		// In TLS 1.3, change_cipher_spec records are ignored.
...
func (c *Conn) readFromUntil(r io.Reader, n int) error {
	// Grow the reusable read buffer once per connection, then reuse it.
	// This eliminates the make([]byte, n) allocation on every TLS record read.
	// Only grows, never shrinks — optimal for steady-state throughput.
	if cap(c.readBuf) < n {
...
// handshake is complete (TLS 1.3 only).
